...
(define (list-partitions total)
   (define (helper total part new)
      (cond ((= total 0) (cons new nil))
            ((< total 0) nil)
            ((= part 0) nil)
            (else (append (helper (- total part) part (append new (cons part nil))) (helper total (- part 1) new)))
      ))
   (filter (helper total total nil) nil)
   )

(define (filter lists new)
      (cond ((null? lists) new)
            ((consecutive? (car lists)) (filter (cdr lists) new))
            (else (filter (cdr lists) (append new (cons (car lists) nil))))
      ))


(define (consecutive? lst)
    (cond ((or (null? lst) (null? (cdr lst))) #f)
          ((= 1 (- (car lst) (cadr lst))) #t)
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               (params (cadr expr))
               (body   (cddr expr)))
                   (cons form (cons params (analyze body)))     ; (cons form (cons (analyze params) (analyze body)))
          ))
        ((let? expr)
         (let ((values  (apply-to-all analyze (cadr expr)))
               (body    (apply-to-all analyze (cddr expr)))
               (params  (apply-to-all analyze (car (zip (cadr expr)))))
               (numbers (apply-to-all analyze (cadr (zip (cadr expr))))))
                    (cons (cons 'lambda (cons params body)) numbers)
          ))
        (else
          (cons (car expr) (apply-to-all analyze (cdr expr)))
        )
  )
)
