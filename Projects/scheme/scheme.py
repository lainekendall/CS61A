...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("number of formal parameters does not equal number of values")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        check_form(expressions, 2, 2)       #check length is 2 exactly
        value = scheme_eval(expressions[1], env)
        env.define(target, value)               # assign value to target
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        symbol = target.first
        arguments = target.second
        body = expressions.second
        lambda_expression = Pair(arguments, body)       
        env.define(symbol, do_lambda_form(lambda_expression, env))
        return symbol
    else:
...
def do_if_form(expressions, env):
    ...
    predicate = scheme_eval(expressions[0], env)
    if scheme_true(predicate):
        consequent = scheme_eval(expressions[1], env, True)
        return consequent
    else:
        if len(expressions) == 3:
            alternative = scheme_eval(expressions[2], env, True)        # tail context
            return alternative
        return okay
...
def do_and_form(expressions, env):
    ...
    for i in range(length - 1):
        predicate = scheme_eval(expressions[i], env)
        if scheme_false(predicate):
            return False
    final_expression = scheme_eval(expressions[length - 1], env, True)      # tail context
    if scheme_true(final_expression):
...
def make_let_frame(bindings, env):
    ...
    for i in range(len(bindings)):
        check_form(bindings[i], 2, 2)
        formal = bindings[i][0]
        if type(formal) != str:
            raise SchemeError("cannot assign value to " + str(type(formal)))
        val = scheme_eval(bindings[i][1], env)
...
def check_formals(formals):
    ...
    copy = formals
    for i in range(len(formals)):       
        if not scheme_symbolp(formals[i]):      # check that each formal is not an integer or list
            raise SchemeError("wrong type")
        if not isinstance(copy, Pair):          # check that formals is well-formed
            raise SchemeError("ill-formed")
        copy = copy.second
    if len(set(formals)) != len(formals):       # check for repeats
        raise SchemeError("repeats")
