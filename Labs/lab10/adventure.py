...
def adv_parse(line):
    ...
    if token == 'give':
        person_index = tokens.index('to') + 1
        obj = tokens[0:person_index-1]
        return (token, ' '.join(obj), tokens[person_index])
    elif token == 'ask':
       obj_index = tokens.index('for') + 1
       return (token, tokens[0], ' '.join(tokens[obj_index:]))
...
def adv_eval(exp):
    ...
    if is_person(exp):
        ...
        thing = exp[1]
        return give(person, thing)
    elif exp[0] == 'ask':
        person_name = exp[1]
        person = Person.people[person_name]
        message = exp[2]
        return ask(person, message)
    else:
        operator = exp[0]
        thing = exp[1]
        return adv_apply(adv_eval(operator), exp[1])
